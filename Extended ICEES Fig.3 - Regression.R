...
# hist.graph <- cloud(EDcount~MajorRoadwayHighwayExposure+MaxDailyPM2.5Exposure, final.df,
...
lininterp <- ols(EDcount~MajorRoadwayHighwayExposure+MaxDailyPM2.5Exposure, data=final.df)
bplot(Predict(lininterp, MajorRoadwayHighwayExposure=1:6, MaxDailyPM2.5Exposure=1:5), 
      lfun=wireframe,  # bplot passes extra arguments to wireframe
      screen = list(z = 50, x = -80), drape=TRUE, add=TRUE)
