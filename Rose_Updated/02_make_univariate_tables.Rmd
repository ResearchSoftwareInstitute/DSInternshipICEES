...
In this file, we extract the univariate tables for each feature. The output of this file is the data frame `univariate_df`.
...
for(i in 1:length(univariate.list[[1]])) {
  if(i == 1) {
    univariate_df <- make_df(i)
  } else {
    univariate_df <- rbind(univariate_df,make_df(i))
  }
}
```

This data frame is called `univariate_df`. Here are the results for the first two features.
...
```{r}
length(unique(univariate_df$feature))
```