...
uni.call <- httr::GET(url,
          accept_json(),
          content_type("application/json"),
          encode = "json")
## the next line of code extracts just the content as a character string
uni.text <- httr::content(uni.call, "text")
## convert the output to JSON format
## note that this contains all of the variables in the dataset
uni.list <- jsonlite::fromJSON(uni.text, flatten = TRUE)$`return value`
```

Note that the current data object, `uni.list`, has the univariate binned distribution for all of the features.
...
```{r}
uni.list$feature_matrix[[1]]
```

The second component of the output contains the names of the features. We show the first five names here.

```{r}
cat("Number of Features = ", length(uni.list$feature.feature_name),"\n")
uni.list$feature.feature_name[1:5]
```
...
```{r, message=FALSE}
uni.list$feature.feature_qualifiers[[1]]
```
...
```{r, message=FALSE}
uni.list$feature.feature_qualifiers[[2]]
```
...
make_df <- function(i) {
  data.frame(feature=uni.list$feature.feature_name[i],
             uni.list$feature.feature_qualifiers[i],
              uni.list$feature_matrix[[i]])
}
## this code loops through the entire list and returns a single data frame
## pdh: should remember how to do this more elegantly!
for(i in 1:length(uni.list[[1]])) {
  if(i == 1) {
    uni_df <- make_df(i)
  } else {
    uni_df <- rbind(uni_df,make_df(i))
  }
}
```

This data frame is called `uni_df`. Here are the results for the first two features.
